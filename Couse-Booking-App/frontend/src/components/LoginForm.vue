<template>
	<div class="lg:w-[500px] flex flex-col gap-[40px]">
		<div class="user-form-container w-full">
			<Form :validation-schema="schema" @submit="handleLogin" class="w-full flex flex-col gap-[20px]">
				<!-- Server message -->
				...
				<div class="flex flex-col gap-[12px]">
					...
					<Field name="email" type="text" placeholder="myemail@gmail.com"
						class="bg-[#EFEFEF] text-[#A5A4A4] rounded-lg block placeholder:text-sm p-2.5 focus:outline-none w-full" />
					<ErrorMessage name="email" class="error-message" />
				</div>

				<!-- Password-->
				<div class="flex flex-col gap-[12px]">
					<label for="password" class="block text-base font-medium text-dark-purple">
						Password
					</label>
					<Field name="password" type="password" placeholder="••••••••"
						class="bg-[#EFEFEF] text-[#A5A4A4] rounded-lg block p-2.5 focus:outline-none w-full" />
					<ErrorMessage name="password" class="error-message" />
				</div>

				<div class="w-full flex flex-col gap-[10px]">
					<button type="submit"
						class="w-full !mt-[10px] text-white bg-gradient-to-br from-[#7786EB] to-[#7786EB] 
...
const handleLogin = (values) => {
	emit("login", values); // { email, password }
};
</script>
