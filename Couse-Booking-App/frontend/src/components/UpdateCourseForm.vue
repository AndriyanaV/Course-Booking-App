<template>
	<div class="w-[1200px] bg-white py-[20px] px-[40px] rounded shadow">
		<div class="form wraper w-full flex flex-col gap-[40px]">
			<form class="w-full" @submit.prevent="uploadCourse()">
...
<script setup>
	...
	const getCourseInfo = async () => {
		try {
			...
			console.log(course.value.language);
			name.value = course.value.name;
			language.value = course.value.language;
			imagePreview.value = course.value.course_image_url;
		} catch (error) {
			toast.error(error);
		}
	};
